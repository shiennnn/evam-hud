...
void loadMapInContainer() {
  ...
  if (map_img == nullptr) {
    ...
    lv_obj_set_size(map_img, 300, 300);
  ...
  lv_img_set_zoom(map_img, (300 * 256) / 256);  // ✅ Fixed: (target/source * 256)
  lv_obj_set_size(map_img, 300, 300);

  lv_obj_invalidate(map_img);

  Serial.println("Image set to lv_img.");

  // DON'T free img_data yet - LVGL is still using it!
  // Store it to free later if needed
