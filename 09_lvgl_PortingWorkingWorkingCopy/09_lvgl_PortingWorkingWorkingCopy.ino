...
const char* SPOTIFY_CLIENT_ID = "145fc21d9c574d25b2ae328c00ebe7ad";
const char* SPOTIFY_CLIENT_SECRET = "37cefc9a84d148afa44d08673f6f8e5f";
const char* SPOTIFY_REFRESH_TOKEN = "BQDEmKM7zc_B_lvsCcdQOgBphUwPijj3SE9tApiWbDvMXtFuo3YOqfPhqKfWUzwqNgZiXiFMdcNOcIx5n6u3MCU3zoq2wST0yYWmHwIyzfUQ8XAYUolhRqNKt-jG10hY_5Oiq5vr_g1pHl2iSN6GPyAWLLuF7KDV2I2ChUQzCAzTZ65MPrnC_4xrYTnoWbl_MXuljAHQhTIFYyT4ovArkxwwT1ZU4vfTJOnPg0mWYPJsGiUrdGT5dm6xZ1s";
const char* SPOTIFY_TOKEN_URL = "https://accounts.spotify.com/api/token";
const char* SPOTIFY_API_BASE = "https://api.spotify.com/v1";
...
double map_lat = 1.34136;
double map_lon = 103.96231;
...
void stateofdrive() {
  ...
  if (bytesReceived > 0) {
    ...
    if (switchState != lastSwitchState) {
      ...
      // Send CAN message immediately when state changes
      update_i2c_reverse(switchState);
      handle_tx_message();
      lvgl_port_lock(-1);
...
void loadMapInContainer() {
  ...
  // Scale using zoom (256 = 100%, so MAP_SCALE * 256)
  // CRITICAL: Set antialias OFF to prevent edge artifacts
  lv_img_set_antialias(map_img, false);

  // Use pivot point for cleaner scaling
  lv_img_set_pivot(map_img, 0, 0);

  // Apply zoom - convert MAP_SCALE to LVGL zoom units (256 = 1.0x)
  int zoom_level = (int)(MAP_SCALE * 256);
